...
:  Node("vesc_interface", options)
{
  ...
  vesc_interface_ = std::make_unique<vesc_interface::VescInterface>(wheel_diameter_, motor_wheel_ratio_, max_steer_angle_);
  vesc_speed_pub_ = this->create_publisher<std_msgs::msg::Float64>("/commands/motor/speed", 10);
  vesc_servo_position_pub_ = this->create_publisher<std_msgs::msg::Float64>("/commands/servo/position", 10);

  control_command_sub_ = this->create_subscription<autoware_auto_control_msgs::msg::AckermannControlCommand>(
  ...
  emergency_command_sub_ = this->create_subscription<tier4_vehicle_msgs::msg::VehicleEmergencyStamped>(
    "/control/command/emergency_cmd", 10,
    std::bind(&VescInterfaceNode::emergency_command_callback, this, std::placeholders::_1));
}

void VescInterfaceNode::control_command_callback(const autoware_auto_control_msgs::msg::AckermannControlCommand::SharedPtr msg)
{
...
void VescInterfaceNode::gear_command_callback(const autoware_auto_vehicle_msgs::msg::GearCommand::SharedPtr msg)
{
  vesc_interface_->set_current_gear((Gear)msg->command);
}

void VescInterfaceNode::emergency_command_callback(const tier4_vehicle_msgs::msg::VehicleEmergencyStamped::SharedPtr msg)
{
